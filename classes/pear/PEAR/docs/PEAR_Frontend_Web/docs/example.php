...
if ($env=getenv('PHP_PEAR_INSTALL_DIR')) {
    ...
    putenv('PHP_PEAR_INSTALL_DIR=C:\xampp\htdocs\/care2002_tz_mero_vps\classes\pear/PEAR');
}

if ($env=getenv('PHP_PEAR_BIN_DIR')) {
    define("PHP_PEAR_BIN_DIR",$env);
} else {
    putenv('PHP_PEAR_BIN_DIR=C:\xampp\htdocs\/care2002_tz_mero_vps\classes\pear');
}
if ($env=getenv('PHP_PEAR_PHP_BIN')) {
    define("PHP_PEAR_PHP_BIN",$env);
} else {
    putenv('PHP_PEAR_PHP_BIN=C:\xampp\htdocs\/care2002_tz_mero_vps\classes\pear');
}
...
ini_set('include_path', 'C:\xampp\htdocs\/care2002_tz_mero_vps\classes\pear/PEAR');
$useDHTML         = true;
