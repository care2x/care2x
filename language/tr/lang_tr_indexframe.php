<?php
$LDHome='Ba�lang��';
$LDPatient='Hasta';
$LDAdmission='Hasta Kabul ';
$LDAmbulatory='Poliklinikler';
$LDMedocs='T�bbi belgeler';
$LDDoctors='Doktorlar';
$LDNursing='Servisler';
$LDOR='Ameliyathane';
$LDLabs='Laboratuvarlar';
$LDRadiology='Radyoloji';
$LDPharmacy='Eczane';
$LDMedDepot='Ambar';
$LDDirectory='Telefon Rehberi';
$LDTechSupport='Bak�m Onar�m';
$LDEDP='Bilgi ��lem';
$LDIntraEmail='Intranet Eposta';
$LDInterEmail='Internet Eposta';
$LDSpecials='�zel Ara�lar';
$LDLogin='Giri�';
$LDLogout='��k��';
$LDAppointments='Randevular';

$VersionChgTarget='_�st';
$LDLanguage='Dil';
$LDPlsWaitInit='Program ilk defa ba�lat�l�yor. L�tfen bekleyiniz.';

$LDChange='De�i�tir';
/* 2003-04-27 EL */
$LDBrazilian='Brezilyaca';
$LDCzech='�ekoslovakyaca';
$LDEnglish='�ngilizce';
$LDFrench='Frans�zca';
$LDGerman='Almanca';
$LDIndonesian='Endonezyaca';
$LDItalian='�talyanca';
$LDNorwegian='Norve�ce';
$LDPolish='Polonyaca';
$LDPortuguese='Portekizce';
$LDSpanish='�spanyolca';
$LDTurkish='T�rk�e';
/* 2003-04-28 El */
$LDDutch='Hollandaca';
# 2003-08-04 EL
$LDPerson='Ki�i';
# 2003-09-05 EL
$LDUser='Kullan�c�';
[?][>]