    ...
    if($ergebnis = $db->Execute($sql)) {
     ...
     switch($mode) {
			case 'ack_print':
    			...
    			//end:gjergji
    		 	...
    		 	if($cat=='medlager'){
    		 		...
    		 		while ($dept = $ergebnis->FetchRow()) { 
    					$dstmp = explode('/',$dept['expiry']);
    					$date_expiry = $dstmp[2] . '-' . $dstmp[1] . '-' . $dstmp[0];						
    					$sqlpharmacy = "";	
    					$sqlpharmacy = "INSERT INTO $tableupdatepharmacy(pcs,expiry,price,bestellnum,idcare_pharma) VALUES (" . $dept['pcs'] . ",'". $dept['expiry']. "','". $dept['price']. "','".$dept['bestellnum']. "','" . $dept_nr . "')";
    			...
    			while ($meds = $ergebnis->FetchRow()) { 
    				if($cat == 'pharma') {
    					$sql = "INSERT INTO $dbhistorik(id_repart, data, bill_nr, medikament, quantity, price, value, date_expiry, id_sub) ";
    					$sql .= "values ( " . $dept_nr. ",'" . $meds['order_date'] . "'," . $order_nr . ",'" . $meds['bestellnum'] . "'," . $meds['pcs'] . ",". $meds['price'] . "," . $meds['pcs'] * $meds['price'] . ",'" . $meds['expiry'] . "'," . $meds['idsub']. ")";
    				} else {
    					$sql = "INSERT INTO $dbhistorik(id_farmaci, data, bill_nr, medikament, quantity, price, value, date_expiry, id_sub) ";
    					$sql .= "values ( " . $dept_nr. ",'" . $meds['order_date'] . "'," . $order_nr . ",'" . $meds['bestellnum'] . "'," . $meds['pcs'] . ",". $meds['price'] . "," . $meds['pcs'] * $meds['price'] . ",'" . $meds['expiry'] . "'," . $meds['idsub']. ")";
    				}
...
$i=1;
    ...
    while ($meds = $ergebnis->FetchRow()) {    
    	...
    	echo'<font face=Arial size=2 >'.$i.'</td>
    		...
    		<td align="right"><font face=Verdana,Arial size=2> &nbsp;'.$meds['expiry'].'</td>
    		<td align="right"><font face=Verdana,Arial size=2> &nbsp;'.$meds['bestellnum'].'</td>
				</tr>';
	$i++;
    	$grand_total = $grand_total + $meds['price'] * $meds['pcs'];
 	}
	echo "<tr bgcolor=\"#ffffff\"><td colspan=10>&nbsp;</td></tr>";
	echo "<tr bgcolor=\"#ffffff\"><td colspan=6></td><td><b>Totali : </b></td><td align=right ><b>".$grand_total."</b></td><td colspan=2><b> Lek</b></td></tr>";
